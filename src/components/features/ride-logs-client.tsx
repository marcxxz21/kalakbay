...
import { CalendarCheck, Clock, Pencil, Star, Trash2 } from "lucide-react";
...
export function RideLogsClient() {
  ...
  const [editingLog, setEditingLog] = useState<RouteLog | null>(null);
  const [editLogSignal, setEditLogSignal] = useState<number | undefined>(undefined);
  ...
  function editLog(log: RouteLog) {
    setEditingLog(log);
    setEditLogSignal((signal) => (signal ?? 0) + 1);
  }
      ...
      <DarkCard className="overflow-hidden p-0">
        ...
        <div className="divide-y divide-white/[0.06]">
          ...
          {filteredLogs.map((log) => (
            <div key={log.id} className="grid gap-3 px-5 py-4 text-sm md:grid-cols-[minmax(0,1fr)_120px_120px_90px_88px] md:items-center">
              <div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  className="flex size-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.065] text-white transition hover:bg-white/[0.095]"
                  onClick={() => editLog(log)}
                  aria-label={`Edit log for ${log.ll_saved_routes?.route_name ?? "saved route"}`}
                  title="Edit log"
                >
